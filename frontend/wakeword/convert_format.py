...
def convert_mp3_to_pcm(source_file, target_file):
    ...
    try:
        ...
        print(
            f"Converted format: {audio.channels} channels, {audio.sample_width*8}-bit, {audio.frame_rate}Hz"
        )
        print(
            f"Original duration: {original_duration}ms, New duration with padding: {len(audio)}ms"
        )
        ...
        print(
            f"Successfully converted to PCM: {os.path.basename(target_file)} ({len(pcm_data)} bytes)"
        )
        return True
