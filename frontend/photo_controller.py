...
class PhotoController(QObject):
    ...
    def process_media_path(self, media_path, is_video):
        ...
        if is_video:
            ...
            return self.photo_processor.add_shadow_effect(media_path, getattr(self, 'is_dark_mode', False))
    
    def find_blurred_background(self, image_path):
        """Find or create a blurred background version of the current image"""
        if not image_path or not os.path.exists(image_path):
            logger.error(f"Cannot find blurred background: Invalid image path {image_path}")
            return
    ...
    def advance_to_next(self):
        ...
        # Emit the new media item
        self.currentMediaChanged.emit(processed_path, is_video)
        logger.debug(f"Showing media {self.current_index + 1}/{len(self.media_files)}: {processed_path}")
