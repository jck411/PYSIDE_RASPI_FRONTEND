...
class ConfigManager:
        ...
        try:
            if os.path.exists(self._user_config_path):
                with open(self._user_config_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded user configuration from {self._user_config_path}"
                    )
            else:
    ...
    def load_module_config(
        self, module_path: str, module_key: str, config_vars: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        if module_key in self._module_configs:
            ...
            logger.debug(
                f"Module config for key '{module_key}' already loaded. Skipping load from {module_path}."
            )
        ...
        try:
            ...
            logger.info(
                f"Loaded module config '{module_key}' from {module_path}: {', '.join(config_vars)}"
            )
            return config_dict
            ...
            logger.error(
                f"Error loading module config '{module_key}' from {module_path}: {e}"
            )
            # Store an empty dict to prevent repeated load attempts for this key
            self._module_configs[module_key] = {}
            return {}
    ...
    def get_config(self, path: str, default: Any = None) -> Any:
        ...
        override_value = None
        if (
            "module_overrides" in self._file_configs.get("user", {})
            and source in self._file_configs["user"]["module_overrides"]
            and var in self._file_configs["user"]["module_overrides"][source]
        ):
            ...
            if len(parts) > 2:
                ...
                for i, key in enumerate(keys_to_traverse):
                    if (
                        isinstance(current_override_level, dict)
                        and key in current_override_level
                    ):
                        current_override_level = current_override_level[key]
                        logger.debug(
                            f"[get_config] Traversing override key '{key}' for {path}, current value: {current_override_level}"
                        )
                    else:
                        logger.debug(
                            f"[get_config] Nested key '{key}' not found in override for {path}. Fallback likely."
                        )
                ...
                if all_keys_found:
                    ...
                    logger.debug(
                        f"[get_config] Found nested override for {path}: {override_value}"
                    )
            else:
                ...
                logger.debug(
                    f"[get_config] Found direct override for {path}: {override_value}"
                )
        ...
        if override_found:
            ...
            logger.debug(
                f"[get_config] No override found for path '{path}'. Checking module config."
            )
        ...
        if source in self._module_configs and var in self._module_configs[source]:
            ...
            if len(parts) > 2:
                ...
                for key in keys_to_traverse:
                    if (
                        isinstance(current_module_level, dict)
                        and key in current_module_level
                    ):
                        current_module_level = current_module_level[key]
                        logger.debug(
                            f"[get_config] Traversing module key '{key}' for {path}, current value: {current_module_level}"
                        )
                    else:
                        logger.debug(
                            f"[get_config] Nested key '{key}' not found in module config for {path}. Using default."
                        )
                        all_keys_found = False
            ...
            self._config_cache[path] = config
            logger.debug(
                f"[get_config] Found value in module config for {path}: {config}"
            )
            return config

        logger.debug(
            f"[get_config] Path '{path}' not found in overrides or module config. Returning default: {default}"
        )
        return default
    ...
    def set_config(self, path: str, value: Any) -> bool:
            ...
            for key in parts[1:-1]:  # Iterate up to the second-to-last part
                ...
                if key == var and len(parts) == 3:  # e.g., stt.STT_CONFIG.enabled
                    ...
                    if isinstance(override_target[var], dict):
                        ...
                        logger.warning(
                            f"Overwriting non-dictionary value at '{'.'.join(parts[:-1])}' with dictionary for key '{parts[-1]}'"
                        )
                        override_target[var] = {parts[-1]: value}
                    break  # Value set, exit loop

                # Otherwise, traverse deeper
                if key not in override_target or not isinstance(
                    override_target.get(key), dict
                ):
                    logger.debug(
                        f"[set_config] Creating/overwriting dict for key '{key}' in override target"
                    )
        ...
        logger.debug(
            f"[set_config] User config state before saving for path '{path}': {self._file_configs.get('user', {})}"
        )
    ...
    def reset_config(self, path: str) -> bool:
        ...
        if (
            "module_overrides" not in self._file_configs["user"]
            or source not in self._file_configs["user"]["module_overrides"]
            or var not in self._file_configs["user"]["module_overrides"][source]
        ):
            # Nothing to reset
            return True
        ...
        if (
            source in self._file_configs["user"]["module_overrides"]
            and not self._file_configs["user"]["module_overrides"][source]
        ):
            del self._file_configs["user"]["module_overrides"][source]

        if (
            "module_overrides" in self._file_configs["user"]
            and not self._file_configs["user"]["module_overrides"]
        ):
